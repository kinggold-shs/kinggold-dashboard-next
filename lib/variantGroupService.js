import { gidToNumericId, shopifyGraphql } from './shopifyProductLookup';
...
/** @param {{ values?: string[], optionValues?: { name?: string }[] }} option */
export function optionValueNamesFromGraphql(option) {
  const names = new Set();
  for (const node of option?.optionValues || []) {
...
/** @param {{ id?: number|string, name?: string, values?: string[] }[]} restOptions */
/** @param {{ id?: string, name?: string, position?: number, values?: string[], optionValues?: { name?: string }[] }[]} gqlOptions */
export function enrichRestProductOptions(restOptions, gqlOptions) {
  if (!gqlOptions?.length) return restOptions;

  const byId = new Map();
  const byName = new Map();
  for (const opt of gqlOptions) {
    const numericId = gidToNumericId(opt.id);
    if (numericId != null) byId.set(String(numericId), opt);
    if (opt?.name) byName.set(String(opt.name).trim(), opt);
  }

  return (restOptions || []).map(rest => {
    const gql = byId.get(String(rest.id)) || byName.get(String(rest.name || '').trim());
    if (!gql) return rest;
    const fromNodes = (gql.optionValues || []).map(v => String(v?.name || '').trim()).filter(Boolean);
    const merged = fromNodes.length
      ? fromNodes
      : [...optionValueNamesFromGraphql(gql)];
    const numericGqlId = gidToNumericId(gql.id);
    return {
      ...rest,
      id: numericGqlId != null ? numericGqlId : rest.id,
      position: gql.position ?? rest.position,
      values: merged.length ? merged : (rest.values || []),
    };
  });
}

export async function fetchProductOptionsGraphql(domain, token, productId) {
  const data = await shopifyGraphql(domain, token, PRODUCT_OPTIONS_GRAPHQL, {
...
function mapRestVariant(variant, productOptions) {
  ...
  return {
    ...
    sku: variant.sku || '',
    title: variant.title || '',
    inventory_quantity: variant.inventory_quantity != null ? Number(variant.inventory_quantity) : null,
...
export async function fetchProductVariants(domain, token, productId) {
  ...
  let productOptions = Array.isArray(product.options) ? product.options : [];
  try {
    const gqlOptions = await fetchProductOptionsGraphql(domain, token, productId);
    productOptions = enrichRestProductOptions(productOptions, gqlOptions);
  } catch {
    // REST-only fallback when GraphQL is unavailable
  }
