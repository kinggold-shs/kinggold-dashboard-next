...
/** @param {import('./shopifyProductLookup.js').shopifyGraphql} graphql */
export async function fetchProductVariants(domain, token, productId, graphql) {
  const query = `
  ...
  `;
  const data = await graphql(domain, token, query, {
    id: `gid://shopify/Product/${productId}`,
  });
  const product = data?.product;
  if (!product) return null;
  const variants = (product.variants?.edges || []).map(({ node }) => {
    const selectedOptions = (node.selectedOptions || []).map(o => ({
      name: o.name || '',
      value: o.value || '',
    }));
    const optionValues = selectedOptions.map(o => o.value).filter(Boolean);
    return {
