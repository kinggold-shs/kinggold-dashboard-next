 ...
 *
 * Priority matches GHS17 and the price formula in GWEB (first match wins):
 * gco_pr_us > prcus > gco_pr > prc. The catalog (item-card) charge outranks
 * the piece-level one; reading only prc/prcus made this card show a stale
 * piece charge while GWEB billed the catalog charge.
 */
export function formatFn6SellChargePerGram(item) {
  if (Number(item?.gco_pr_us) > 0) return `$${Number(item.gco_pr_us).toFixed(2)} /g`;
  if (Number(item?.prcus) > 0) return `$${Number(item.prcus).toFixed(2)} /g`;
  if (Number(item?.gco_pr) > 0) return `${formatFn6Currency(item.gco_pr)} /g`;
