...
export async function syncAllChainsToShopify(domain, token, productId, payload, mco) {
  ...
  const originalActiveByKey = new Map(
    (payload.chains || []).map(chain => [chain.key, getActiveCode(chain)]),
  );
  const syncedChains = [];
  for (const chain of payload.chains || []) {
    ...
    const previousActiveCode = originalActiveByKey.get(chain.key) ?? null;
    if (newActiveCode && newActiveCode !== previousActiveCode) {
      try {
        await refreshVariantPrice(newActiveCode);
      } catch {
        // best-effort: a price refresh failure must not abort the sync
      }
    }
  }
