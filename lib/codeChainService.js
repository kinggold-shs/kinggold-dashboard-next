...
import { refreshVariantPrice } from './refreshVariantPrice';
...
async function createOrUpdateVariantForCode(domain, token, productId, {
  ...
  if (price == null) {
    console.error(`[codeChainService] no computable price for ${code} (FN6 item.price=${item?.price}) — variant will be created/updated WITHOUT a price field, risking a 0.00 Shopify price`);
  }
  const inventoryBody = {
  ...
  if (existing) {
    ...
    if (price != null) variant.price = String(price);
...
export async function setVariantAvailability(domain, token, variantId, available) {
...
export async function syncAllChainsToShopify(domain, token, productId, payload, mco) {
  ...
  for (const chain of payload.chains || []) {
    ...
    // Always refresh the live price for every synced chain (not just when the
    // active code changed) so each variant's Shopify price is authoritative
    // on every sync, and log failures instead of swallowing them — a silent
    // failure here previously left variants stuck at Shopify's default 0.00.
    const newActiveCode = getActiveCode(synced);
    if (newActiveCode) {
      try {
        await refreshVariantPrice(newActiveCode);
      } catch (e) {
        console.error(`[syncAllChains] price refresh failed for ${newActiveCode}:`, e?.message || e);
      }
    }
  }
...
/**
 * Unpublish a product to `draft` when every chain is exhausted (no active code
 * AND no further queued codes). Every entry in `soldCodes` is treated as a
 * confirmed sale because the only writer is `advanceChain`, which is only
 * called from the `orders/paid` webhook or a manual advance — so no extra
 * sale-record lookup is needed.
 *
 * @param {string} domain
 * @param {string} token
 * @param {string|number} productId
 * @param {CodeChain[]} chains
 * @returns {Promise<{ unpublished: boolean }>}
 */
export async function unpublishProductIfFullySoldOut(domain, token, productId, chains) {
  const list = Array.isArray(chains) ? chains : [];
  if (list.length === 0) return { unpublished: false };

  const fullySoldOut = list.every(chain => getActiveCode(chain) == null);
  if (!fullySoldOut) return { unpublished: false };
  ...
  if (!res.ok) {
    const errMsg = typeof data.errors === 'object'
      ...
      : (data.errors || 'Failed to unpublish product');
