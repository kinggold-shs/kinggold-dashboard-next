...
/**
 * Manufacturer charge per gram, per line item.
 *
 * A Shopify line item only carries SKU / title / qty / price — the per-gram
 * manufacturer charge (`prc` in EGP, `prcus` in USD) lives in GWEB, and the
 * paid snapshot doesn't copy it either. So look each SKU up by mco. Cached
 * because the same code recurs across orders and pages, and these numbers only
 * change when the item is re-costed.
 */
const MFG_CACHE_TTL_MS = 5 * 60 * 1000;
const MFG_LOOKUP_CONCURRENCY = 8;
const mfgCache = new Map(); // sku -> { at, value: { prc, prcus, go_cr } | null }
  ...
  try {
    ...
    if (item) {
      value = {
        ...
        prcus: item.prcus != null ? Number(item.prcus) : null,
        go_cr: item.go_cr != null ? Number(item.go_cr) : null,
      };
    }
  } catch {
...
async function enrichItemsWithMfgPerGram(rows) {
  ...
  for (const row of rows) {
    ...
    row.items = row.items.map((item) => {
      ...
      if (!mfg) return item;
      // Named `prc` / `prcus` to match FN6 so the dashboard's shared
      // formatFn6MfgPerGram helper renders these the same as the scan page.
      return { ...item, prc: mfg.prc, prcus: mfg.prcus, weight_g: mfg.go_cr };
    });
  }
