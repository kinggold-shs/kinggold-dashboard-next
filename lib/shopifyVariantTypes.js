...
export async function ensureSubVariantDiscriminatorOption(
  ...
  const customerOptionCount = options.filter(
    o => o?.name
      && !isPlaceholderOptionName(o.name)
      && !isGwebWeightOptionName(o.name)
      && !isSubVariantDiscriminatorOption(o.name),
  ).length;
  if (customerOptionCount >= 3) return;

  if (options.length >= 3) {
    const err = new Error(
      'Cannot add sub-variant: product already has 3 option types and no Code slot for duplicate selections.',
    );
    err.statusCode = 400;
    throw err;
  }
...
export async function fetchVariantOptionSuggestions(domain, token, limit = 80) {
  ...
  for (const { node } of data?.products?.edges || []) {
    for (const opt of node?.options || []) {
      ...
      if (isGwebWeightOptionName(name) || isSubVariantDiscriminatorOption(name)) continue;
