      ...
      edges {
        node {
          ...
          product {
            ...
            variants(first: 1) {
              edges { node { id sku inventoryQuantity inventoryItem { tracked } } }
            }
          }
        }
      }
...
function parseProductCollections(product) {
  return (product.collections?.edges || []).map(({ node }) => ({
...
function packFromVariant(variant) {
  ...
  return {
    ...
    tags: product.tags || [],
    collections: parseProductCollections(product),
...
/**
 * Fast SKU-only lookup for bulk price sync: single GraphQL query, no
 * barcode/title/handle fallbacks. Returns `{found, variantId}`.
 * ~300ms per call vs up to 4s for the full `findShopifyProduct` strategy
 * sweep, which is critical for staying under Vercel's 10s function timeout.
 */
export async function findShopifyVariantBySkuOnly(domain, token, sku) {
  const code = String(sku || '').trim();
  if (!code) return { found: false };
  const data = await shopifyGraphql(domain, token, FAST_VARIANT_QUERY, { query: `sku:${code}` });
