 ...
 * @param {object|null} [params.prefetchedProduct=null] - Pre-fetched product data; skips extra API call when provided.
...
export async function cleanupProductVariantDiscriminators({
  ...
  const product = prefetchedProduct || await fetchProductVariants(domain, token, productId);
  ...
  if (hasSuffix && !hasCode && customerTypes.length >= 3) {
    ...
    if (!dryRun && changes.length && lastField) {
      for (const change of changes) {
        ...
        // Rate limit: 600ms between each variant write.
...
export async function cleanupAllProductVariantDiscriminators({ domain, token, dryRun = true }) {
  ...
  for (const product of products) {
    ...
    if (!firstVariantSku) {
      ...
      try {
        // Pass the pre-fetched listing data so no extra per-product API call is needed.
        // Dry-run analysis is now free; apply-phase rate limiting is per variant write.
