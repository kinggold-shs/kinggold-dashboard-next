...
/**
 * Enforces uniqueness of the **last** customer-facing option value across all variants.
 * Only applied when there are >= 2 customer option types. Earlier types (Karat, 2nd) repeat freely.
 * @param {{ name: string }[]} optionTypes
 * @param {Record<string, string>} selectedByName
 * @param {{ id?: number|string, option1?: string, option2?: string, option3?: string }[]} variants
 * @param {{ id?: number|string } | null} mainVariant
 * @param {{ excludeVariantId?: number|string | null, shopifyOptions?: { name?: string, position?: number }[] | null }} [opts]
 * @returns {string | null}
 */
export function validateLastOptionUniqueness(
  optionTypes,
  selectedByName,
  variants,
  mainVariant,
  ...
  const { excludeVariantId = null, shopifyOptions = null } = opts;
  const customerTypes = filterCustomerOptionTypes(optionTypes);
  if (customerTypes.length < 2) return null;

  // Determine "last" type by Shopify position when available (highest position = last slot).
  // Fall back to the orderVariantTypes canonical rank when no position data exists.
  let sortedCustomerTypes;
  if (shopifyOptions?.length) {
    sortedCustomerTypes = [...customerTypes].sort((a, b) => {
      const posA = resolveOptionFieldIndex(shopifyOptions, a.name);
      const posB = resolveOptionFieldIndex(shopifyOptions, b.name);
      const rankA = posA >= 0 ? posA : 99;
      const rankB = posB >= 0 ? posB : 99;
      return rankA - rankB;
    });
  } else {
    sortedCustomerTypes = orderVariantTypes(customerTypes);
  }

  const lastType = sortedCustomerTypes[sortedCustomerTypes.length - 1];
  const rawValue = selectedByName?.[lastType.name];
  if (!rawValue) return null;
  const selectedValue = stripOptionValueForDisplay(String(rawValue).trim()).toLowerCase();
  ...
  for (const variant of allVariantsForOptionScan(variants, mainVariant)) {
    ...
    const payload = variantToOptionPayload(variant, sortedCustomerTypes, shopifyOptions);
    const existing = payload[lastType.name];
    if (!existing) continue;
    if (existing.trim().toLowerCase() === selectedValue) {
      return `${lastType.name} "${stripOptionValueForDisplay(String(rawValue).trim())}" is already used by another variant. The last variant type must be unique to identify each item.`;
    }
  }
