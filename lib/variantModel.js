 ...
 * Karat and Weight may match on main + subs; Size and other types must be unique per variant.
 * @param {string} optionName
 */
export function allowsSharedOptionValueAcrossVariants(optionName) {
  const n = String(optionName || '').trim().toLowerCase();
  return n === 'karat' || n === 'weight';
}
 ...
 * @returns {{ name: string, values: string[] }[]}
 */
export function defaultVariantTypesForNewProduct(karatValues = ['18K', '21K', '24K']) {
