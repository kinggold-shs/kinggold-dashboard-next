 ...
 * @returns {string}
 */
export function applyShopifyOnlyOptionSuffix(displayValue, sku) {
  const val = String(displayValue || '').trim();
  const skuStr = String(sku || '').trim();
  if (!val || !skuStr) return val;
  const base = stripShopifyOnlyOptionSuffix(val, skuStr);
  return `${base}${SUB_VARIANT_VALUE_SUFFIX_SEP}${skuStr}`;
}
...
/**
 * When a customer-option combo matches an existing variant, disambiguate for Shopify:
 * Code option when <3 customer types; SKU suffix on last type when 3 customer types.
 * @param {object} params
 * @returns {{ selectedByName: Record<string, string>, discriminatorApplied: boolean, suffixApplied: boolean, error: string | null }}
 */
export function resolveSubVariantOptionSelections({
  ...
  const customerTypes = orderVariantTypes(filterCustomerOptionTypes(optionTypes));
  const needsDiscriminator = hasDuplicateCustomerOptionCombo(
    next,
    variants,
    shopifyOptions,
    optionTypes,
  ...
  if (!needsDiscriminator) {
    return { selectedByName: next, discriminatorApplied: false, suffixApplied: false, error: null };
  }
  ...
  if (!skuValue) {
    return {
      ...
      suffixApplied: false,
      error: 'SKU is required when option selections match an existing variant.',
    };
  }

  if (customerTypes.length >= 3) {
    const lastType = customerTypes[customerTypes.length - 1];
    const displayVal = next[lastType.name];
    if (!displayVal) {
      return {
        selectedByName: next,
        discriminatorApplied: false,
        suffixApplied: false,
        error: `Select a value for ${lastType.name} before saving a duplicate combo.`,
      };
    }
    next[lastType.name] = applyShopifyOnlyOptionSuffix(displayVal, skuValue);
    return { selectedByName: next, discriminatorApplied: false, suffixApplied: true, error: null };
  }

  next[SUB_VARIANT_DISCRIMINATOR_OPTION] = skuValue;
  return { selectedByName: next, discriminatorApplied: true, suffixApplied: false, error: null };
}
...
export function resolveOptionCatalogValues(type, shopifyOptions, variants, mainVariant) {
  ...
  for (const variant of allVariantsForOptionScan(variants, mainVariant)) {
    ...
    const v = raw && !isPlaceholderOptionValue(raw)
      ? stripShopifyOnlyOptionSuffix(String(raw).trim(), variant?.sku)
      : '';
    if (v && !seen.has(v)) {
      seen.add(v);
      fromVariants.push(v);
...
export function getOptionSelectUiState({
  ...
  return {
    selectableValues: catalog,
    hint,
    disableSelect: !catalog.length,
  };
}
