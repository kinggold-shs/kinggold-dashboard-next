...
export function unionVariantTypesWithLiveValues(types, variants, shopifyOptions) {
  ...
  const expanded = (types || []).map(t => {
    ...
    if (!extra?.size) return t;
    return {
      ...t,
      values: [...new Set([...(t.values || []), ...extra])],
    };
  });
...
/** Auto-assigned Shopify option3 when Karat+Size repeat across sub-variants (FN6 SKU). */
 ...
 * Karat and Size may match on main + subs; other types must be unique per variant.
 * @param {string} optionName
 * @param {{ name?: string, position?: number }[]} [shopifyOptions]
 */
export function allowsSharedOptionValueAcrossVariants(optionName, shopifyOptions) {
  return isKaratOption(optionName) || isSizeOption(optionName, { shopifyOptions });
}
...
/**
 * Karat + Size combo key for duplicate detection (Shopify requires unique option1/2/3).
 * @param {Record<string, string>} selectedByName
 * @param {{ name?: string, position?: number }[]} shopifyOptions
 * @returns {string}
 */
...
export function hasDuplicatePrimaryOptionCombo(
  ...
  const targetKey = primaryOptionComboKey(selectedByName, shopifyOptions);
  if (!targetKey || targetKey === '\0') return false;

  for (const variant of variants || []) {
    if (excludeVariantId != null && Number(variant.id) === Number(excludeVariantId)) {
      continue;
    }
    if (variantPrimaryOptionComboKey(variant, optionTypes, shopifyOptions) === targetKey) {
      return true;
    }
  }
  return false;
...
/**
 * When Karat+Size matches an existing variant, set Code (option3) to the FN6 SKU so Shopify accepts the create/update.
 * @param {object} params
 * @returns {{ selectedByName: Record<string, string>, discriminatorApplied: boolean, error: string | null }}
 */
export function resolveSubVariantOptionSelections({
  selectedByName,
  sku,
  variants,
  shopifyOptions,
  ...
  const next = { ...(selectedByName || {}) };
  const needsDiscriminator = hasDuplicatePrimaryOptionCombo(
    next,
    variants,
    shopifyOptions,
    optionTypes,
  ...
  if (!needsDiscriminator) {
    return { selectedByName: next, discriminatorApplied: false, error: null };
  }
  ...
  if (!skuValue) {
    return {
      ...
      error: 'SKU is required when Karat and Size match an existing variant.',
    };
  }

  next[SUB_VARIANT_DISCRIMINATOR_OPTION] = skuValue;
  return { selectedByName: next, discriminatorApplied: true, error: null };
}
...
export function getOptionSelectUiState({
  ...
  if (allowsSharedOptionValueAcrossVariants(typeName, shopifyOptions)) {
  ...
  const disableSelect =
    !catalog.length || (!selectableValues.length && !allowsSharedOptionValueAcrossVariants(typeName, shopifyOptions));
...
/**
 * Enforces unique option values per variant for non-shared dimensions (not Karat or Size).
 * @param {{ name: string }[]} optionTypes
 * @param {Record<string, string>} selectedByName
...
export function validateNonKaratOptionUniqueness(
  ...
  for (const type of optionTypes || []) {
    if (allowsSharedOptionValueAcrossVariants(type.name, shopifyOptions)) continue;
    const value = selectedByName?.[type.name];
    if (!value) continue;
