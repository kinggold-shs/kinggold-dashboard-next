...
export async function fetchCodeChains(productId, mco = '') {
  const params = mco ? `?mco=${encodeURIComponent(String(mco))}` : '';
  const res = await fetch(`/api/shopify/products/${encodeURIComponent(String(productId))}/code-chains${params}`);
  const data = await parseApiJson(res);
  if (!res.ok) throw new Error(data.error || 'Failed to load code chains');
  return data;
}

export async function saveCodeChains(productId, codeChains, mco = '') {
  const res = await fetch(`/api/shopify/products/${encodeURIComponent(String(productId))}/code-chains`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ codeChains, mco }),
  });
  const data = await parseApiJson(res);
  if (!res.ok) throw new Error(data.error || 'Failed to save code chains');
  return data;
}

export async function advanceCodeChain(productId, chainKey, mco = '') {
  const res = await fetch(`/api/shopify/products/${encodeURIComponent(String(productId))}/code-chains`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ action: 'advance', chainKey, mco }),
  });
  const data = await parseApiJson(res);
  if (!res.ok) throw new Error(data.error || 'Failed to advance chain');
  return data;
}

export async function migrateCodeChains(productId, mco = '') {
  const res = await fetch(`/api/shopify/products/${encodeURIComponent(String(productId))}/code-chains`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ action: 'migrate', mco }),
  });
  const data = await parseApiJson(res);
  if (!res.ok) throw new Error(data.error || 'Failed to migrate chains');
  return data;
}

export async function fetchShopifyInventoryPreflight() {
  const res = await fetch('/api/shopify/inventory-preflight');
  const data = await parseApiJson(res);
...
export async function repairProductVariants(productId, { mco, dryRun = true }) {
  const res = await fetch(
    `/api/shopify/products/${encodeURIComponent(String(productId))}/repair-variants`,
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ mco, dryRun }),
    },
  );
  const data = await parseApiJson(res);
  if (!res.ok) throw new Error(data.error || 'Failed to repair variant options');
  return data;
}

export async function repairAllProductVariants({ dryRun = true } = {}) {
  const res = await fetch('/api/shopify/products/repair-variants-bulk', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
