...
/**
 * Pure 18K total-price calculation from raw inputs — reusable on both the
 * dashboard and the Shopify theme JS. Mirrors the Gweb serializer formula
 * (Gweb/Sup/api/serializers.py Fn6Serializer.get_price).
 *
 * @param {object} args
 * @param {number|string} args.gold21     21K gold price per gram (gp.pr21)
 * @param {number|string} args.usdRate    USD→EGP rate (Fc2 rate, item.dollar)
 * @param {number|string} args.weight     gold weight in grams (item.go_cr)
 * @param {number|string} [args.prc]      extra EGP charge (item.prc)
 * @param {number|string} [args.prcus]    extra USD charge (item.prcus)
 * @returns {number|null} rounded 18K total price in EGP, or null if inputs invalid
 */
export function compute18kPriceFromInputs({ gold21, usdRate, weight, prc, prcus } = {}) {
  const g21 = toNum(gold21);
  const rate = toNum(usdRate) ?? 1;
  const w = toNum(weight) ?? 0;
  if (g21 == null) return null;
  const gold18 = (g21 * 6) / 7;
  const p = toNum(prc) ?? 0;
  const pu = toNum(prcus) ?? 0;
  let tot;
  if (pu > 0) {
    const totus = (pu + gold18 / rate) * w;
    tot = totus * rate;
  } else if (p > 0) {
    tot = (p + gold18) * w;
  } else {
  ...
  return Math.sign(tot) * Math.floor(Math.abs(tot) + 0.5);
}
